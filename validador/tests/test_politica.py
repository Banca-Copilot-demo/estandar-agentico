"""La politica de promocion: quien entra al marketplace y quien no.
...
def test_lo_certificado_se_distribuye_con_cualquier_politica(promocion):
    ...
    Una politica que pudiera excluirlo dejaria el marketplace vacio sin que nadie lo hubiera pedido."""
    assert entra_al_marketplace(ESTADO_CERTIFICADO, promocion)


@pytest.mark.parametrize("estado", ["suspended", "deprecated", "retired"])
@pytest.mark.parametrize("promocion", list(Promocion), ids=[p.value for p in Promocion])
def test_ningun_estado_de_salida_entra_al_marketplace(estado, promocion):
    """Suspendido, Obsoleto y Retirado salen del marketplace por su propio flujo. Si la politica de
    promocion pudiera devolverlos, una suspension urgente se desharia sola al republicar."""
    assert not entra_al_marketplace(estado, promocion)


# ── lo que si cambia ────────────────────────────────────────────────────────────────────────
def test_con_al_certificar_lo_conforme_NO_se_distribuye():
    assert not entra_al_marketplace(ESTADO_AL_PUBLICAR, Promocion.AL_CERTIFICAR)
...
def test_con_al_publicar_lo_conforme_SI_se_distribuye():
    ...
    assert entra_al_marketplace(ESTADO_AL_PUBLICAR, Promocion.AL_PUBLICAR)
...
@pytest.mark.parametrize("politica", [
    None,
    {},
    {"promocion_al_marketplace": None},
    {"promocion_al_marketplace": ""},
    {"promocion_al_marketplace": "al_publicar_todo"},
    {"promocion_al_marketplace": "AL_CERTIFICAR"},
...
def test_una_politica_declarada_se_respeta(promocion):
    assert promocion_declarada({"promocion_al_marketplace": promocion.value}) is promocion

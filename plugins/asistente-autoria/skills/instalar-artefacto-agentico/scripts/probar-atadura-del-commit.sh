...
# conexion: consulta el CATALOGO REAL con `gh`, asi que necesita credencial. Es una prueba de
# integracion, no unitaria, y por eso vive junto a los scripts y no en la suite rapida del dominio.
#
# Se hace asi a proposito: lo que se quiere fijar es que el catalogo publicado y el comprobante se
# comparan de verdad. Un doble del catalogo probaria la comparacion pero no que la consulta funcione
# -- y el defecto que motivo todo esto fue justamente un dato que nadie contrastaba con su fuente --.
# Lo unico que se prueba sin red es la distincion entre «catalogo ilegible» y «artefacto ausente»,
...
# El catalogo se sustituye por un DOBLE inyectado por entorno (T4): la prueba no toca la red ni
# parchea el modulo. `instalar.sh` lo consulta a traves de `commit_que_el_catalogo_instalaria`.
comprobar() {  # comprobar <descripcion> <esperado> <directorio> <nombre en catalogo>
...
comprobar "el commit del catalogo COINCIDE -> instala" \
  "$ESPERADO_CONTINUA" "$trabajo/correcto" demo.sdlc.referencia
comprobar "el commit del catalogo DIFIERE -> bloquea" \
  "$ESPERADO_BLOQUEA" "$trabajo/trucado" demo.sdlc.referencia
comprobar "artefacto AUSENTE del catalogo -> bloquea" \
  "$ESPERADO_BLOQUEA" "$trabajo/correcto" demo.sdlc.no-esta-en-el-catalogo
comprobar "comprobante SIN commit_atestado -> bloquea" \
  "$ESPERADO_BLOQUEA" "$trabajo/sin-commit" demo.sdlc.referencia

# Un catalogo ILEGIBLE no es lo mismo que un artefacto ausente, y confundirlos seria fail-open: un
# fallo de red haria pasar por «no distribuido» algo que si lo esta, o al reves.
codigo_ilegible=0
salida_ilegible="$(
  source "$AQUI/_comun.sh"
  REPO_CATALOGO="Banca-Copilot-demo/repositorio-que-no-existe-xyz"
  commit_que_el_catalogo_instalaria demo.sdlc.referencia
)" || codigo_ilegible=$?
if [ "$codigo_ilegible" -eq 2 ] && [ -z "$salida_ilegible" ]; then
  echo "  ok    catalogo ILEGIBLE se distingue de artefacto ausente (codigo 2)"
else
  echo "  FALLO catalogo ilegible: esperado codigo 2 y salida vacia; obtenido $codigo_ilegible '$salida_ilegible'"
  fallos=$((fallos + 1))
fi
